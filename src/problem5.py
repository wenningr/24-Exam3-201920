...
def problem5(seq_of_seq):
    ...
    for k in range(len(seq_of_seq)):
        high_number = -math.inf
        for j in range(len(seq_of_seq[k])):
            if seq_of_seq[k][j] > high_number:
                high_number = seq_of_seq[k][j]
        if high_number != -math.inf:
            new_list = new_list + [high_number]
    return new_list
